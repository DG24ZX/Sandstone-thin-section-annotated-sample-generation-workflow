...
def generate_xpl_pore_texture(
    ...
    """
    Generate XPL pore texture: near-black, low-brightness texture.
    """
    h, w = img_size
    ...
    for _ in range(num_dark_spots):
        ...
        cv2.ellipse(img, (x, y), axes, angle, 0, 360, (c, c, c), -1)
    ...
    for _ in range(num_light_spots):
        ...
        cv2.circle(img, (x, y), radius, (c, c, c), -1)
...
def apply_dissolution_effect(
    ...
    XPL dissolution areas are set to near black.
    PPL dissolution areas are filled with pore-like colors.
    """
    ...
    for y, x in zip(ys, xs):
        out_ppl[y, x] = adjust_brightness_random(ppl_base_color)
...
def addClay(
    ...
    This is a cleaned single-XPL version of the original addClay/addClay2 logic.
    """
    random_trend = np.random.uniform(1, 8, size=edges.shape)
    random_trend_smooth = cv2.GaussianBlur(random_trend, (49, 49), sigmaX=29)
    random_trend_smooth = cv2.normalize(random_trend_smooth, None, 1, edges_max, cv2.NORM_MINMAX)

    edges_dilated = np.zeros_like(edges, dtype=np.uint8)

    ys, xs = np.where(edges == 255)
    for i, j in zip(ys, xs):
        kernel_size = int(random_trend_smooth[i, j])
        kernel_size = max(1, kernel_size)
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (kernel_size, kernel_size))

        y_start = max(0, i - kernel_size // 2)
        y_end = min(edges.shape[0], i + kernel_size // 2 + 1)
        x_start = max(0, j - kernel_size // 2)
        x_end = min(edges.shape[1], j + kernel_size // 2 + 1)

        region = edges_dilated[y_start:y_end, x_start:x_end]
        local_dilation = cv2.dilate((edges[y_start:y_end, x_start:x_end] > 0).astype(np.uint8), kernel)
        edges_dilated[y_start:y_end, x_start:x_end] = np.maximum(region, local_dilation)

    distance_transform = cv2.distanceTransform(edges_dilated, cv2.DIST_L2, 5)
    mask = (distance_transform >= 2).astype(np.uint8)
    edges_dilated = edges_dilated * mask

    # Smooth immediate edges first.
    kernel = np.ones((5, 5), np.uint8)
    edges_for_smooth = cv2.dilate(edges, kernel, iterations=2)

    thin_cross_smooth = thin_cross.copy()
    thin_single_smooth = thin_single.copy()

    smoothed_cross = cv2.GaussianBlur(thin_cross_smooth, (5, 5), 0)
    thin_cross_smooth[edges_for_smooth == 255] = smoothed_cross[edges_for_smooth == 255]

    smoothed_single = cv2.GaussianBlur(thin_single_smooth, (5, 5), 0)
    thin_single_smooth[edges_for_smooth == 255] = smoothed_single[edges_for_smooth == 255]

    out_cross = thin_cross_smooth.copy()
    out_single = thin_single_smooth.copy()

    clay_color = np.clip(np.array(colorValue, dtype=np.float32) * color_scale, 0, 255).astype(np.uint8)
    clay_pixels = (edges_dilated == 1) & (np.random.rand(*edges_dilated.shape) <= fill_probability)

    out_cross[clay_pixels] = (30, 30, 30)

    # Mix clay color and dark pixels in PPL.
    dark_choice = np.random.rand(*edges_dilated.shape) < 0.35
    color_pixels = clay_pixels & (~dark_choice)
    dark_pixels = clay_pixels & dark_choice

    ys, xs = np.where(color_pixels)
    for y, x in zip(ys, xs):
        out_single[y, x] = adjust_brightness_random(clay_color, min_factor=0.8, max_factor=1.2)

    out_single[dark_pixels] = (40, 40, 40)
